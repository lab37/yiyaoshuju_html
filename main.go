...
func chaxun(w http.ResponseWriter, r *http.Request) {
  ...
  if biaoge == "zhongbiao" { 
	    ...
	    shengchanqiye := strings.TrimFunc(keys["shengchanqiye"][0],unicode.IsSpace)
		dishi := strings.TrimFunc(keys["dishi"][0],unicode.IsSpace)
		type Rst struct {
		  Tym string
		  Jx string
		  Gg string
		...
		if yaopinname=="" && shengchanqiye=="" && dishi=="" {
		    rst := []Rst {{"你未输入任何有效的字符串", " ", " ", " ", " ", " ", " ", " ", " ", " "}}
		    r, _ := json.Marshal(rst)
		    fmt.Fprintf(w, string(r))
		} else {
			...
			for rows.Next() {
			  rst := Rst{}
			  err = rows.Scan(&rst.Tym, &rst.Jx, &rst.Gg, &rst.Scqy, &rst.Ds, &rst.Yy, &rst.Fzr, &rst.Lxfs, &rst.Rylx, &rst.Bz)
			  checkErr(err)
			  rsts = append(rsts, rst)
			}
			r, _ := json.Marshal(rsts)
